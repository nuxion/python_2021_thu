"""
Tomar el valor del dolar oficial y dolar blue y guardar un archivo .txt con los siguientes datos:

    2020-04-08;95.5;140.0,44.5
    2020-04-09;95.5;140.0,43.5

Cada columna significa lo siguiente:
    fecha;dolar_blue;dolar_oficial;diferencia


Como ayuda uno de los lugares donde se puede tomar informacion es de:

https://www.dolarsi.com/api/api.php?type=valoresprincipales

+ info: https://www.reddit.com/r/argentina/comments/e63ezq/necesito_una_api_para_el_valor_del_d%C3%B3lar/




"""


def print_name():
    print(__name__)



